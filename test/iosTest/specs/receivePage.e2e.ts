...
describe('Receive Page', () => {
    ...
    // Receive Token Page Test Scenario
    describe('', () => {
        beforeAll(async () => {
            await action.click(component.selectTokenKUBTokenBtn)
        })
        ...
        beforeEach(async () => {
            AllureReporter.addSubSuite('Receive Token Page Test Scenario')   // add sub suite for allure report
        })
        ...
        it('[Functional] => Check Bottom Wording When Change Network', async () => {
            // action
            await customNetworkModel.addCustomNetwork(`${process.env.GOERLI_TEST_NET_URL}`)
            await action.click(component.homeReceiveBtn)
    ...
    // Deny Photo Permission Warning Modal Test Scenario
    describe('', () => {
        beforeAll(async () => {
            await action.click(component.homeReceiveBtn)    // go to receive page
            await action.click(component.selectTokenKUBTokenBtn)     // select KUB
        ...
        beforeEach(async () => {
            AllureReporter.addSubSuite('Deny Photo Permission Warning Modal Test Scenario')   // add sub suite for allure report
        })
    ...
    describe('', () => {
        ...
        beforeEach(async () => {
            AllureReporter.addSubSuite('Set Amount Page Test Scenario')   // add sub suite for allure report
        })
        ...
        xit('[Function] => Check Continue with 10 KUB', async () => {
            // action
            await action.enterText(component.setAmountTextField, ' ')   // reset states keyboard
            await action.click(component.setAmountTextField)    // open keyboard
